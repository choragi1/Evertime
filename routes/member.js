...
router.post("/add", function (req, res) {
  ...
  if (
    ...
    Counter.findOne({ name: "member" }, function (err, result) {
...
router.put("/password", (req, res) => {
  ...
  let checkPw = req.body.checkPw;
  let reg_pw = /(?=.*\d)(?=.*[a-zA-ZS]).{8,}/;

  User.findOne({ _id: userNo }, (err, user) => {
    bcrypt.compare(currentPw, user.pw, (err, result) => {
      if ((currentPw == null) | (currentPw.length == 0)) {
        res.send("현재 비밀번호를 입력해주세요.");
      } else if ((changePw == null) | (changePw.length == 0)) {
        res.send("새 비밀번호를 입력해주세요.");
      } else if ((checkPw == null) | (checkPw.length == 0)) {
        res.send("새 비밀번호 확인을 입력해주세요.");
      } else if (!reg_pw.test(currentPw)) {
        res.send('비밀번호는 문자,숫자를 1개 이상 포함한 8자리 이상만 가능합니다.')
      } else if (result != true) {
        res.send("비밀번호가 틀렸습니다.");
      } else {
        bcrypt.hash(changePw, 10, (err, hash) => {
          User.updateOne(
            { _id: userNo },
            { $set: { pw: hash } },
            (err, updateRes) => {
  ...
  User.findOne({ _id: userNo }, (err, user) => {
    bcrypt.compare(userPW, user.pw, (err, result) => {
      if (result != true) {
        ...
        res.send(
          "비밀번호는 문자,숫자를 1개 이상 포함한 8자리 이상만 가능합니다."
        );
      } else if (!reg_nick.test(userNick)) {
        res.send("닉네임은 2~10자리 내의 한글,영문만 사용가능합니다.");
      } else if (!reg_name.test(userName)) {
  ...
  User.findOne({_id : userNo},(err,user)=>{
    bcrypt.compare(password,user.pw, (err,result)=>{
      if(result!=true){
        ...
        User.deleteOne({_id : userNo}, function (err, result) {
